...
def to_complex(struct_file1, struct_file2, complex_file):
    ...
    with open(complex_file, 'w') as outfile:
        for line in lines1:
            assert line.startswith('ATOM') # equikdock doesn't process HETATM records
...
def calc_ires(pdb_file, chain1, chain2, p1, p2, resmapping1, resmapping2, p1_len, p2_len, uSASA=15.0, dSASA=1.0):
    ...
    with open(pdb_file, 'r') as infile:
        for line in infile:
            ...
            if line.startswith('ATOM'): # equikdock doesn't process HETATM records
    ...
    for chain in pdbatomdict:
        ...
        for line in naccess_output:
            ...
            if all_atoms_rel >= uSASA:      # relative percent SASA >= 15% indicates surface residue
                residue_key = (res, chain, res_num)
                asadict[chain][residue_key] = all_atoms_abs   #save absolute SASA to compare with bound form
...
if __name__ == '__main__':
    ...
    equidock_pred_path = '../data/equidock_preds/'
    equidock_complex_path = '../data/equidock_pred_complexes/'
    equidock_ires_path = '../data/equidock_ires/'
    
    start = int(sys.argv[1])
    end = int(sys.argv[2])
    for interaction in all_interactions[start:end]:
        ...
        if p1 == p2:
            ...
            for i, p1_struct in enumerate(p1_structs):
                if p1_struct[0] == 'PDB':
                    ...
                    f_name2 = p1+'_AF'
                struct_file1 = equidock_pred_path+f_name1+'.pdb'
                struct_file2 = '../data/structs/'+f_name2+'.pdb'
                complex_file = equidock_complex_path+f_name1+'_complex.pdb'
                ...
                with open(equidock_ires_path+f_name1+'.pkl', 'wb') as outfile:
