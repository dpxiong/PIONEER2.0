...
def make_flat1(points):
    d = points.shape[-1]
    points = points.reshape([-1, d])
    return points

def make_flat2(points):
    d = points.shape[-1]
    points = points[points.max(-1).max(-1) > 0].reshape([-1, d])
    return points
...
if __name__ == '__main__':
    ...
    for struct_basefile in struct_basefiles[start:end]:
        ...
        aa_activity = get_activity(make_flat1(local_coordinates['aa_local_coordinates']), aa_centers, aa_sqrt_precision_matrix, eps=0.1, covariance_type='full')
        atom_activity = get_activity(make_flat1(local_coordinates['atom_local_coordinates']), atom_centers, atom_sqrt_precision_matrix, eps=0.1, covariance_type='full')
#                 aa_activity = get_activity(make_flat2(local_coordinates['aa_local_coordinates']), aa_centers, aa_sqrt_precision_matrix, eps=0.1, covariance_type='full')
#                 atom_activity = get_activity(make_flat2(local_coordinates['atom_local_coordinates']), atom_centers, atom_sqrt_precision_matrix, eps=0.1, covariance_type='full')
