...
#     print(np.expand_dims(coordinates1[...,0],axis=-1))
#     print(np.expand_dims(coordinates2[...,0],axis=-2))
#     print(np.expand_dims(coordinates1[...,0],axis=-1) - np.expand_dims(coordinates2[...,0],axis=-2) )
...
def calculate_LocalCoordinates(frames, Kmax=16):
    ...
    local_coordinates = np.sum(np.expand_dims(
        second_centers[neighbors[:,:,0]]  # L x Kmax x 3
        - np.expand_dims(first_centers, axis=-2),  # L X 1 X 3,
        axis=-2)  # L X Kmax X 1 X 3
        * np.expand_dims(frames[:,1:4], axis=-3)  # L X 1 X 3 X 3
        , axis=-1)  # L X Kmax X 3
...
if __name__ == '__main__':
    ...
    for struct_basefile in struct_basefiles[start:end]:
        ...
        with open(result_dir+'local_neighborhood_frames/'+struct_basefile[:-4]+'_local_neighborhood_frames.pkl', 'wb') as outfile:
