...
class Network(nn.Module):
    ...
    def __init__(self, config):
        ...
        kernels_pre = net_params['kernels_pre']
        kernels_mid_last = net_params['kernels_mid_last']
        channels = net_params['channels']
        self.prediction_padding = net_params['prediction_padding']
        ...
        self.layers = nn.ModuleList([GraphTransformerLayer(hidden_dim, hidden_dim, num_heads, 
                                                           dropout, layer_norm, batch_norm, residual) for _ in range(n_layers-1)])
        self.layers.append(GraphTransformerLayer(hidden_dim, hidden_dim, num_heads, dropout, layer_norm, batch_norm, residual))
        
        self.transformer_net = Transformer_net(in_dim_node, num_heads_trans, hidden_dim_trans, n_layers_trans, dropout_trans)
        self.cnn_conv_pre = CNNConvLayersPre(kernels_pre)
