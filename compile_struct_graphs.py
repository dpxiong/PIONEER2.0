...
# def compile_graphs(p1, p2, p_index, structs1, structs2, label1, label2, len_p1, len_p2, struct_dir, gmm_dir, preppi_info):
def compile_graphs(p1, p2, p_index, structs1, structs2, len_p1, len_p2, struct_dir, gmm_dir, preppi_info):
    if p1 == p2:
#         target_batch_graphs, target_batch_aa_gmms, target_batch_atom_gmms, target_batch_atom_nums, batch_labels, batch_seq_ids, partner_batch_graphs, partner_batch_aa_gmms, partner_batch_atom_gmms, partner_batch_atom_nums, batch_nums = [], [], [], [], [], [], [], [], [], [], []
...
#             batch_labels.append(label1[seq_id])
...
#         return [target_batch_graphs, batch_labels, batch_seq_ids, len_p1, partner_batch_graphs, batch_nums], [target_batch_aa_gmms, target_batch_atom_gmms, target_batch_atom_nums, partner_batch_aa_gmms, partner_batch_atom_gmms, partner_batch_atom_nums]
...
#             batch_labels.append(label[target_seq_id])
...
#         return [target_batch_graphs, batch_labels, batch_seq_ids, len_p, partner_batch_graphs, batch_nums], [target_batch_aa_gmms, target_batch_atom_gmms, target_batch_atom_nums, partner_batch_aa_gmms, partner_batch_atom_gmms, partner_batch_atom_nums]
...
if __name__ == '__main__':
    ...
    result_dir1 = '../data/struct_graphs_input'
    result_dir2 = '../data/struct_graphs_gmms_input'


    struct_dir = '../data/structs'
    gmm_dir = '../data/gmm_activity'
...
#         print(interaction)
        ...
        if p1 == p2:
            ...
            graphs, gmms = compile_graphs(p1, p2, None, structs1, None, len(fasta_dict[p1]), None, struct_dir, gmm_dir, preppi_info)
            with open(os.path.join(result_dir1, p1+'_'+p2+'.pkl'), 'wb') as outfile1:
                pickle.dump(graphs, outfile1)
            with open(os.path.join(result_dir2, p1+'_'+p2+'.pkl'), 'wb') as outfile2:
                pickle.dump(gmms, outfile2)
        else:
            structs1 = structures[p1+'_'+p2][p1]
            structs2 = structures[p1+'_'+p2][p2]
            graphs, gmms = compile_graphs(p1, p2, 0, structs1, structs2, len(fasta_dict[p1]), len(fasta_dict[p2]), struct_dir, gmm_dir, preppi_info)
            with open(os.path.join(result_dir1, p1+'_'+p2+'.pkl'), 'wb') as outfile1:
                pickle.dump(graphs, outfile1)
            with open(os.path.join(result_dir2, p1+'_'+p2+'.pkl'), 'wb') as outfile2:
                pickle.dump(gmms, outfile2)
                
            graphs, gmms = compile_graphs(p1, p2, 1, structs1, structs2, len(fasta_dict[p1]), len(fasta_dict[p2]), struct_dir, gmm_dir, preppi_info)
            with open(os.path.join(result_dir1, p2+'_'+p1+'.pkl'), 'wb') as outfile1:
                pickle.dump(graphs, outfile1)
            with open(os.path.join(result_dir2, p2+'_'+p1+'.pkl'), 'wb') as outfile2:
                pickle.dump(gmms, outfile2)
